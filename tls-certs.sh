...
# Create CA cert
openssl req -subj "/CN=eastus.cloudapp.azure.com/O=Microsoft/C=US" -new -x509 -days 365 -passin pass:asdfasdf -key keys/ca-key.pem -sha256 -out keys/ca.pem

# Create CSR for server cert
openssl genrsa -out keys/server-key.pem 4096
openssl req -subj "/CN=dockertest.eastus.cloudapp.azure.com" -sha256 -new -key keys/server-key.pem -passout pass:asdfasdf -out keys/server.csr

# Extend with daemon host/ip
echo subjectAltName = DNS:dockertest.eastus.cloudapp.azure.com,IP:10.10.10.20,IP:127.0.0.1 > keys/extfile.cnf
...
# Base64 encode CA
cat keys/ca-key.pem | base64 > keys/b64_ca.txt
